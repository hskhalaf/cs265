...
\say{A key concept here is \texttt{total\_recomp\_time}. This is the total wall-clock cost of recomputing a tensor across its entire lifetime, accounting for cascading dependencies. The formula is simple: \texttt{total\_recomp\_time = recomp\_time * recomp\_cnt}. \texttt{recomp\_time} is the profiled execution time of the single node --- for example, relu takes 0.06 ms. \texttt{recomp\_cnt} is how many times you'd need to recompute it, starting at 1. So initially, total equals recomp\_time for every intermediate.}

\say{Where it gets interesting: cascading. Say tensor B depends on tensor A --- A is an input to B's computation. If A gets evicted, then to recompute B during backward, you first need to recompute A. So B's effective cost goes up. Before evicting A, B has recomp\_cnt = 1 and total = 0.06 ms. After evicting A, B's recomp\_cnt becomes 2 and total becomes 0.12 ms. B's recompute\_ratio --- memory\_size divided by total\_recomp\_time --- drops by half, making B less attractive to evict next.}

\say{This prevents the algorithm from creating a chain where evicting B would require first recomputing A then B --- compounding costs. The distinction: \texttt{recomp\_time} is the cost of just this one node. \texttt{total\_recomp\_time} is the cost including all the dependencies you'd need to recompute first.}

\qa{Does this mean evicting a widely-used tensor is penalised?}{Yes. If tensor A is a recomp\_src for many other tensors, evicting A increases all of their costs simultaneously. Their ratios all drop, making the algorithm naturally cautious about evicting ``hub'' tensors that many others depend on.}
...
\say{This is the final safety check after the greedy algorithm picks which tensors to evict. Here's the problem: say the algorithm evicts both tensor A and tensor B, but B's computation requires A as an input --- for example, \texttt{B = relu(A)}. During backward, when we need B, we'd try to recompute it. But A was also evicted, so A isn't in memory either. The recomputation would fail.}

\say{The rule: to recompute a tensor, every input to the operation that produced it must be available in GPU memory. Available means either it's a placeholder --- parameters, batch data, these are always in memory --- or it's a retained intermediate, one we chose to keep.}

\say{The loop: scan all evicted nodes. If any evicted node has an evicted input, move it back to the retained set. But moving one node back might fix another node's dependency, so we loop until nothing changes. In practice it converges in one or two iterations.}

\say{Concrete example: say we evicted both \texttt{mm} (a matmul) and \texttt{relu} where \texttt{relu = relu(mm)}. The validation finds that \texttt{relu} depends on \texttt{mm} which is also evicted --- invalid. It moves \texttt{relu} back to retained. Now \texttt{mm}'s own inputs are placeholders (weight matrices), so \texttt{mm} stays evicted. Final result: \texttt{mm} evicted, \texttt{relu} retained.}

\qa{Does this ever undo many evictions?}{Rarely. The greedy algorithm already avoids this via cascading cost propagation --- if evicting A made B expensive, B's ratio dropped and it probably wasn't evicted in the first place. The validation is a safety net for edge cases the ratio didn't catch.}
