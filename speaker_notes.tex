...
\say{The traced graph has a clear structure. At the top, placeholder nodes represent inputs: parameters, optimizer states, and batch data. Then forward operations, a separator node we insert to mark the boundary, loss computation, another separator for the backward boundary, backward operations, and finally the optimizer step. This structure lets us partition everything into four regions.}

\qa{What are the separator nodes?}{SEPFunction is an identity function we wrap around the loss. It generates explicit marker nodes in the graph --- a clean boundary that's guaranteed to be there regardless of the loss function used.}
...
\say{This was one of the trickiest challenges. The starter code uses \texttt{foreach=True} for Adam, which decomposes into hundreds of small ops --- no single optimizer node. And FakeTensors all have \texttt{requires\_grad=False}.}

\say{Our solution: parameters are the only placeholders that appear in \emph{both} the forward pass and the optimizer step. Batch data appears in forward and backward but NOT optimizer. Optimizer states appear only in optimizer. This works reliably for all models.}

\qa{What if a model uses SGD instead of Adam?}{The usage-pattern heuristic still works --- parameters are always updated by the optimizer (forward + optimizer users). The specific optimizer decomposition doesn't matter.}

\slide{Five Tensor Categories}
\say{Five categories matching the course spec: PARAM, ACT, GRAD, OPTIMIZER\_STATE, OTHER. Gradients are only identifiable with fused Adam. Optimizer states are placeholders with optimizer-only users.}
...
\say{CUDA events for timing, memory\_allocated for deltas. We also swap intermediates to CPU during profiling --- this measures swap latency and prevents OOM on large models. The swap happens outside the node timing measurement.}
